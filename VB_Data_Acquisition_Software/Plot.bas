...
Public Sub Plot(frmSpec As Form, dArSpec() As Double, dArSpecx() As Double, udtLayoutSpec As GRAPHIC_LAYOUT)
...
'*************  initialise screen  ******************
  ...
  twp_YTick = 80
  twp_XLeftMargin = 800
  twp_XRightMargin = 300 '200
  twp_YTopMargin = 50 '300
...
nNumTraces = 0
  Select Case udtLayoutSpec.DrawTrace
  Case AS_CONLINE
    val_X = dArSpecx(udtLayoutSpec.lStart)
    val_Y = dArSpec(udtLayoutSpec.lStart)
    ...
    For nI = udtLayoutSpec.lStart + 1 To udtLayoutSpec.lEnd Step 1
      ...
      XRatio = (val_X - val_XMin) / val_XRange
      YRatio = (val_Y - val_YMin) / val_YRange
      frmSpec.Line -(twp_X0 + XRatio * twp_XRange, twp_Y0 - YRatio * twp_YRange), clr_Plot(9)
    Next nI
